	...
	initialize() {
		...
		if (this.settings.lastUsedVersion != this.getVersion()) {
			...
			BdApi.alert("Minespoiler - Changelog", `
				Temporary fix: The fields now get created properly, but the message you use to send the message gets sent too. I will fix this later, but I'm too lazy rn. Just clear the textbox.
			`);
		}
	}
