            ...
            return class MessageFilePreview extends Plugin {
                ...
                async patchMessages(promiseState){
                    ...
                    Patcher.after(MemoMessage, "type", (_, [props], retVal) => {
                        ...
                        if(this.messageCache[messageTxt.id]) return addMessageContent(props, messageTxt.id);
                        request(messageTxt.url, (error, response, body) => {
                            if(!error && response.statusCode == 200) this.messageCache[messageTxt.id] = body;
                            else this.messageCache[messageTxt.id] = "Error while fetching data";
