...
class AccountSwitcher {
    ...
    getDescription() {
        return this.local.plugin.description;
    }
        ...
        if (!this.strings) this.strings = JSON.parse(`{
			...
			"ru": {
				...
				"settings": {
					...
					"useCurrent": "Использовать текущий аккаунт"
				},
    ...
    getSettingsPanel() {
        ...
        setTimeout(() => {
            ...
            for (let lang in this.local.settings.languages) {
                langs.push({
                    title: this.local.settings.languages[lang].name,
                    value: lang,
                    description: this.local.settings.languages[lang].translator
                });
            }
